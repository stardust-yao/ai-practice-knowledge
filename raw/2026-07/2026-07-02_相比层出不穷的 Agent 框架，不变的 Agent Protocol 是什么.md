...
Guardrail 拦截
`HandoffTermination``client.interrupt()`**中断状态**
...
重新`run_stream(task=input)`

新`query()`
**多点中断**
支持（多节点设置`interrupt_before`）
...
`FunctionTool + JSON Schema``Tool（JSON Schema）`**调用约定**`ToolNode 自动执行``requires_action → 客户端执行`
Agent 内部直接调用

Agent 内部直接调用
**结果格式**`ToolMessage`
Function output（字符串）
`FunctionExecutionResult``ToolResult`**发现机制**
构建时`bind_tools()`
...
生产环境断线恢复是刚需
**中断/恢复**
通用`interrupt(payload)` +`resume(value)`
...
[1][https://mp.weixin.qq.com/s/1klXy2fr1pspqRshUjg2dQ](https://mp.weixin.qq.com/s/1klXy2fr1pspqRshUjg2dQ)

[2][https://mp.weixin.qq.com/s/4LvnghM71g0udkHYVvkhwA](https://mp.weixin.qq.com/s/4LvnghM71g0udkHYVvkhwA)
...
[跳转微信打开](https://wechat2rss.xlab.app/link-proxy/?k=7deebc0c&r=1&u=https%3A%2F%2Fmp.weixin.qq.com%2Fs%3F__biz%3DMzIzOTU0NTQ0MA%3D%3D%26mid%3D2247561317%26idx%3D1%26sn%3Dcfeecd649d2312d5331e2b5ffb03a8d0)
